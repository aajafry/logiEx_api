...
export const shipmentsController = {
  create: async (req, res) => {
    try {
      await insertShipmentSchema.parseAsync(req.body);
      const { shipment_id, vehicle_vin, captain_id, status, orders } = req.body;
  ...
  updateByShipmentId: async (req, res) => {
    try {
      await updateShipmentSchema.parseAsync(req.body);
      const { shipmentId } = req.params;
      const { shipment_id, vehicle_vin, captain_id, status, orders } = req.body;
